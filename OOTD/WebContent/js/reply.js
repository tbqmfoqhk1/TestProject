...
$(document).ready(function() {
	 ...
	 * 출력하기 위해 동적으로 요소를 생성하기 때문에 delegate 방식의 이벤트
		...
		$.ajax({
			...
			success: function(resultData, status, xhr) {		
				...
				if($("#replyTable").length == 0) {
					replyTable = $("<div id='replyTable'></div>");
					$("#replyList > td").empty();					
	...
	/* 댓글 수정 버튼이 클릭되면
	 * 댓글을 쓰고 나면 동적으로 요소를 생성하기 때문에 delegate 방식으로 이벤트를
	...
	/* 댓글 삭제 버튼이 클릭되면
	 * 댓글을 삭제하면 동적으로 요소를 생성하기 때문에 delegate 방식으로 이벤트를
	 * 등록해야 한다. 만약 $(".deleteReply").click(function() {}); 형식으로 이벤트를 
	 * 등록했다면 새로운 댓글을 등록한 후에는 클릭 이벤트 처리가 되지 않는다.
	 **/
	$(document).on("click", ".deleteReply", function() {	
		...
		var list_no = $("input[name=list_no]").val();
